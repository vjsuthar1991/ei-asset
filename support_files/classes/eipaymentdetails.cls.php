...
//	var $paid2;
	...
	function getSchoolDataArrayasperZonalManager($connid)
	{
		...
		if($this->CheckUserisManagaer($connid,$name))
		{
			...
			$query="SELECT s.schoolno,s.schoolname,s.city FROM schools s, sales_allotment_master m WHERE s.schoolno=m.schoolCode AND (keyAccount='$name' OR buddyAccount='$name')";		 		
	...
	{
		$arrRet = array();
		$query = "SELECT test_edition,description FROM test_edition_details ORDER BY test_edition DESC LIMIT 3";
		$dbquery = new dbquery($query,$connid);
		//echo $query;
		while($row = $dbquery->getrowarray()) 
		{
	...
	function CheckUserisManagaer($connid,$name)
	{
		...
		$query = "SELECT * FROM marketing ".
				 "WHERE ( name = '".$name."') AND category IN ('ZM','REM','RM','EA','AM','SRM')";
				 /*"OR rm_username = '".$name."' ".
				 "OR addtl_username='".$name."' ".
				 "OR mse_username='".$name."' ".
				 "OR ea_username = '".$name."' )";*/
	...
	function getPaymentDetailsBySchoolCode($connid,$school_code,$test_edition)
	{
		...
		$query = "SELECT school_status.no_of_students,school_status.paid,school_status.amount_payable ".
				 "FROM school_status WHERE school_status.school_code = $school_code ".
				 "AND school_status.test_edition = '".$test_edition."'";
		$dbquery = new dbquery($query,$connid);
		$row = $dbquery->getrowarray();	
		
		$query_payment="Select paid from payment_details WHERE school_code = $school_code ".
				 "AND test_edition = '".$test_edition."'";
		$dbquery_payment=new dbquery($query_payment,$connid);	
		
		$i = 1;
		while($row_payment = $dbquery_payment->getrowarray())
		{
			$payment.="Paid".$i." - ".$row_payment[0]."<br>";	
			$i++;
		}	
		$this->no_of_students = $row["no_of_students"];
		$this->amount_payable = $row["amount_payable"];
		
		echo $this->no_of_students.",".$payment.",".$this->amount_payable.",".$str.",".$balamount;
	...
	function sendMail($connid,$name,$school_code)
	{
		...
		elseif($this->test_edition == 'J')
			$test_edition = "Summer 2010";
		elseif($this->test_edition == 'K')
			$test_edition = "Winter 2010";
	...
	function getAllManageremailsOfTerritory($connid,$school_code)
	{
		...
		$query = "SELECT marketing.name,marketing.email FROM marketing ".
				 ...
				 "WHERE school_status.school_code = '".$school_code."' AND category IN ('ZM','RM','EA','SRM','NSM') AND test_edition = '".$this->test_edition."' ".
				 "AND marketing.email != '' UNION SELECT name,email FROM marketing WHERE category = 'NSM' ";
	...
	{
		$arrRet = array();
		$query = "SELECT sum(payment_info.amount) as sumamount,payment_info.school_code,paid as sumpaid123 FROM payment_info ".
	...
	function getBalancePayment($connid)
	{
		...
		$querySS = "SELECT (amount_payable-paid) as sumpaid123 ".
				 "FROM school_status ".
		...
		//$balanceamount=$rowSS["sumpaid123"];
		if($this->amount == "")
			$balanceamount = $rowSS["sumpaid123"];
		else
