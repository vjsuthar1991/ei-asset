...
class clspettycashadvance
{
	...
	function updateData($connid)
	{
		if($this->id != 0)
		{
			$query = "UPDATE petty_cash_advance SET status='approved',approved_by='".$_SESSION["username"]."',approved_amount = '".$this->total_amount."',approved_dt=CURDATE(),modified_by='".$_SESSION["username"]."',modified_dt=NOW()  WHERE id = '".$this->id."' LIMIT 1";
			$dbquery = new dbquery($query,$connid);
		}
	}
	...
	function getAllPettyCashAdvances($connid,$user="")
	{
		...
		$LeftJoin.= " LEFT JOIN office_master ON office_master.id = petty_cash_advance.office_id ";
		...
		if($user != "")
			$query = "SELECT petty_cash_advance.*,bill_office_number,location,DATE_FORMAT(date,'%d-%m-%Y') as advance_date, 
		...
		while($row = $dbquery->getrowarray())
		{
			$arrRet[$row["id"]] = array("id"=>$row["id"],
										"bill_office_number"=>$row["location"],
	...
	function getAdminLocation($connid)
	{
		$arrRet = array();
		$query = "SELECT userID,location FROM emp_master WHERE desig = 'Admin Executive' AND dept = 'General and Administration' ";
		$query.="UNION SELECT userID,location FROM emp_master WHERE userID = 'madhu' ";
		$dbquery = new dbquery($query,$connid);
		while($row = $dbquery->getrowarray()) 
		{
